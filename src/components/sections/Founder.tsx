...
import { useFoundPortraitEasterEggs } from "@/components/FoundPortraitEasterEggs";
import { site } from "@/content/site";

import { SectionReveal } from "../SectionReveal";

const extraColumnClass =
  "flex flex-col items-center border-t border-border-soft pt-12 text-center lg:border-t-0 lg:border-l lg:pl-12 lg:pt-0 lg:text-left xl:pl-14";

export function Founder() {
  const { marvelRevealed, brunoRevealed } = useFoundPortraitEasterEggs();
  const reduceMotion = useReducedMotion();
  const anyRevealed = marvelRevealed || brunoRevealed;
  const bothExtras = marvelRevealed && brunoRevealed;

  const sectionTagline = bothExtras
    ? site.founderEasterEgg.taglineThree
    : marvelRevealed
      ? site.marvelInvite.sectionTagline
      : site.brunoInvite.sectionTagline;
      ...
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        {anyRevealed ? (
          <div className="flex flex-col items-stretch">
            ...
            <div
              className={`mt-12 grid grid-cols-1 gap-12 border-t border-border-soft pt-12 lg:gap-12 lg:pt-14 ${bothExtras ? "lg:grid-cols-3" : "lg:grid-cols-2"}`}
            >
              ...
              {brunoRevealed ? (
                ...
                >
                  <div className="relative h-48 w-48 shrink-0 overflow-hidden rounded-full border border-border-soft shadow-md ring-1 ring-border-soft md:h-56 md:w-56">
                    <Image
                      src="/brand/1759290553813.jpeg"
                      alt={`${site.brunoInvite.name}, ${site.founder.signatureRole}`}
                      width={800}
                      height={800}
                      className="h-full w-full object-cover object-center"
                      sizes="(max-width: 1024px) 192px, 224px"
                  ...
                  <div className="mt-3 flex flex-col items-center gap-0.5 lg:items-start">
                    <p className="text-sm font-semibold text-foreground">
                      {site.brunoInvite.name}
                    </p>
                    <p className="text-sm text-muted">{site.founder.signatureRole}</p>
                  </div>
                  <p className="mt-6 max-w-xl text-base leading-relaxed text-muted md:text-lg">
                    {site.brunoInvite.bio}
                  </p>
                </motion.article>
              ) : null}
            </div>
            <p className="mx-auto mt-12 max-w-3xl border-t border-border-soft pt-10 text-center text-base font-medium leading-relaxed text-foreground md:text-lg">
              {site.founderEasterEgg.joinPitch}
            </p>
          </div>
        ) : (
